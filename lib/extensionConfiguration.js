...
const { createSmarterEncryptionExceptionRule } = require('./smarterEncryption')
...
async function generateExtensionConfigurationRuleset (
    ...
    if (extensionConfig.features.https?.exceptions?.length > 0) {
        appendRuleResult(createSmarterEncryptionExceptionRule(extensionConfig.features.https.exceptions.map(entry => entry.domain)))
    }
