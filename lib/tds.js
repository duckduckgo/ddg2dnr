...
const inverseCustomActionSymbol = Symbol('customAction')
...
async function generateDNRRulesForTrackerEntry (
    ...
    for (let i = trackerEntryRules.length - 1; i >= 0; i--) {
        ...
        if (customRuleAction) {
            ...
            newRule[inverseCustomActionSymbol] = inverseRule
        }
...
function finalizeDNRRulesAndLookup (startingRuleId, dnrRules) {
    ...
    for (const rule of dnrRules) {
        ...
        if (!rule.condition.requestDomains ||
            ...
            rule.action === 'redirect' || rule[inverseCustomActionSymbol]) {
            const ruleId = nextRuleId++
            rule.id = ruleId
            if (rule[inverseCustomActionSymbol]) {
                inverseCustomActionRules.push(rule[inverseCustomActionSymbol])
