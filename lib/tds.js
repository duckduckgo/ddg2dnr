...
const supportedCustomActions = {
    'block-ctl-yt': { action: 'redirect', inverseAction: 'allow' },
    'block-ctl-fb': { action: 'redirect', inverseAction: 'allow' }
}
...
async function generateDNRRulesForTrackerEntry (
    ...
    for (let i = trackerEntryRules.length - 1; i >= 0; i--) {
        ...
        if (customRuleAction) {
            ...
            const inverseRule = trackerEntryRules[i]
            inverseRule.customAction = inverseRule.action
            inverseRule.action = customRuleAction.inverseAction
...
async function generateTdsRuleset (
    ...
    for (const [trackerDomain, trackerEntry] of Object.entries(tds.trackers)) {
        ...
        if(customActions.length) {
            customActionRules.push(customActions)
        }
