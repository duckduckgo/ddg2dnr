...
const customActionSymbol = Symbol('customAction')
...
async function generateDNRRulesForTrackerEntry (
    ...
    for (let i = trackerEntryRules.length - 1; i >= 0; i--) {
        ...
        if (customRuleAction) {
            ...
            const inverseRule = JSON.parse(JSON.stringify({ customAction: trackerEntryRules[i].action, ...newRule }))
            inverseRule.action = { type: customRuleAction.inverseAction }
            delete inverseRule.id
            newRule[customActionSymbol] = inverseRule
        }
...
function finalizeDNRRulesAndLookup (startingRuleId, dnrRules) {
    ...
    for (const rule of dnrRules) {
        ...
        if (!rule.condition.requestDomains ||
            ...
            rule.action === 'redirect' || rule[customActionSymbol]) {
            const ruleId = nextRuleId++
            rule.id = ruleId
            if(rule[customActionSymbol]) {
                customActions.push(rule[customActionSymbol])
                delete rule[customActionSymbol]
            }
            ruleset.push(rule)
    ...
    return { ruleset, matchDetailsByRuleId, customActions }
}
