...
// enumerates the supported custom actions, and the DNR actions they should map to
const supportedCustomActions = {
...
async function generateDNRRulesForTrackerEntry (
    ...
    const customDefaultAction = supportedCustomActions[defaultAction]
    // Default action is required and must be either 'block' or
    // 'allow' (aka 'ignore').
    if (defaultAction !== 'block' && defaultAction !== 'allow' && !customDefaultAction) {
        console.error('DNR unknown default action', JSON.stringify(trackerEntry))
        return { trackerRules: dnrRules, customActions }
    }
    ...
    if (customDefaultAction) {
        // nope?
        customActions.push(trackerEntry)
    }
    ...
    for (let i = trackerEntryRules.length - 1; i >= 0; i--) {
        ...
        const customRuleAction = supportedCustomActions[ruleAction]
        ...
        if (ruleAction !== 'block' && ruleAction !== 'allow' && typeof supportedCustomActions[ruleAction] === 'undefined') {
            console.warn('DNR unknown action', JSON.stringify(trackerEntry))
            continue
        }
        ...
        if (surrogate) {
            ...
            if (!supportedSurrogateScripts.has(surrogate)) {
                // we should ignore actions for missing surrogates, otherwise breakage can result?
                ruleAction = 'ignore'
                console.warn('unsupported surrogate', trackerDomain, ruleAction)
            } else {
                ruleAction = 'redirect'
                redirectAction = {
        ...
        if (customRuleAction) {
            ...
            const inverseRule = { ...newRule }
            inverseRule.customAction = trackerEntryRules[i].action
            inverseRule.action = { type: customRuleAction.inverseAction }
            // inverseRule.priority = priority
            delete inverseRule.id
            customActions.push(inverseRule)
        }
    ...
    const trackerRules = removeRedundantDNRRules(dnrRules)
    return { trackerRules, customActions }
}
...
async function generateTdsRuleset (
    ...
    for (const [trackerDomain, trackerEntry] of Object.entries(tds.trackers)) {
        ...
        if (typeof tds.entities[trackerEntry.owner.name] === 'undefined') {
            console.warn('now skipping', JSON.stringify(trackerEntry.owner.name, tds.entities[trackerEntry.owner.name]))
            continue
        }
        ...
        const { trackerRules, customActions } = await generateDNRRulesForTrackerEntry(
        ...
        if (customActions.length) {
            customActionRules = customActionRules.concat(customActions)
        }
    ...
    return { ...finalizeDNRRulesAndLookup(startingRuleId, dnrRules), customActionRules }
}
